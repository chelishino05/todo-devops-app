...
Works in both test environment and Docker/Azure
"""
...
# ---------------------------------------------------------
# Static files - Works in both test and production
# ---------------------------------------------------------
...
for path in FRONTEND_PATHS:
    if os.path.exists(path) and os.path.isdir(path):
        frontend_dir = path
        logger.info(f"Found frontend directory at: {path}")
        break
...
@app.get("/")
async def root():
    """Serve the frontend HTML or fallback message"""
    if frontend_dir:
    ...
    return HTMLResponse(content="""
        <html>
            <head><title>Todo List Manager</title></head>
            <body>
                <h1>Todo List Manager API</h1>
